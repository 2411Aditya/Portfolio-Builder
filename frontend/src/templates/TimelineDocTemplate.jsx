...
import {
  ...
  Mail, Globe, Sparkles, ChevronRight, Check
} from 'lucide-react';
      ...
      <div style={{ maxWidth: 880, margin: '0 auto' }}>
        ...
        <header style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap', fontSize: 13 }}>
            {contact.email && (
              <a href={`mailto:${contact.email}`} style={{ color: primaryColor, textDecoration: 'none', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <div style={{ position: 'relative', paddingLeft: 32, borderLeft: `3px solid ${primaryColor}` }}>
          ...
          {/* Milestone Items (Experience + Education + Projects) */}
          ...
          {skills.length > 0 && (
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ background: cardBg, border: `1px solid ${borderColor}`, borderRadius: 16, padding: 24 }}>
                <h3 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Award size={18} style={{ color: primaryColor }} /> Technical Capabilities
                </h3>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {skills.map((s, i) => (
                    <span key={i} style={{ padding: '6px 14px', borderRadius: 8, background: isDark ? 'rgba(255,255,255,0.05)' : '#fef3c7', fontSize: 13, fontWeight: 600 }}>
                      {s}
                    </span>
                  ))}
                </div>
