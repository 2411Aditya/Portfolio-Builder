...
import { Mail, Phone, Link2, GitBranch, Globe, ExternalLink, Briefcase, Code2, GraduationCap, Award, User, Zap, ChevronDown, MessageCircle } from 'lucide-react';
...
function ExpItem({ exp }) {
  return (
    <div className="exp-item" style={{ marginBottom: 24 }}>
  ...
  return (
    <div className="proj-card">
      <div className="proj-card-header">
        ...
        {project.url && (
          <a href={project.url} target="_blank" rel="noreferrer" className="proj-card-link"><ExternalLink size={15} /></a>
        )}
      </div>
  ...
  return (
    <div className="dp-root">
      ...
      <section id="hero" className="dp-hero">
        <div className="orb float"  style={{ top: '25%', left: '20%', width: 480, height: 480, background: 'rgba(139,92,246,0.13)' }} />
        <div className="orb float-2" style={{ bottom: '20%', right: '20%', width: 380, height: 380, background: 'rgba(59,130,246,0.11)' }} />
        <div className="hero-grid" />

        <div style={{ position: 'relative', zIndex: 1, maxWidth: 700, margin: '0 auto' }}>
          <div className="dp-avatar">{data?.name?.[0]?.toUpperCase() || '?'}</div>
          <h1>{data?.name || 'Your Name'}</h1>
          <div className="dp-hero-title grad">{data?.title || 'Professional Title'}</div>
          ...
          <div className="dp-socials">
            ...
            {contact.linkedin && <a href={contact.linkedin} target="_blank" rel="noreferrer" className="dp-social-link"><Link2 size={15} style={{ color: '#60a5fa' }} />LinkedIn</a>}
            {contact.github   && <a href={contact.github}   target="_blank" rel="noreferrer" className="dp-social-link"><GitBranch size={15} />GitHub</a>}
            {contact.website  && <a href={contact.website}  target="_blank" rel="noreferrer" className="dp-social-link"><Globe size={15} style={{ color: '#22d3ee' }} />Website</a>}
          </div>
        </div>

        <div className="anim-bounce" style={{ position: 'absolute', bottom: 28, left: '50%', transform: 'translateX(-50%)', color: 'rgba(255,255,255,0.2)' }}>
          <ChevronDown size={20} />
        </div>
      </section>
      ...
      <div className="dp-content">
        {data?.bio && (
          <section id="about" className="dp-section">
            ...
            <div className="dp-card"><p style={{ fontSize: 16, color: 'rgba(255,255,255,0.65)', lineHeight: 1.8 }}>{data.bio}</p></div>
          </section>
        )}
          ...
          <section id="certifications" className="dp-section">
            ...
            <div className="cert-grid">
              {data.certifications.map((c, i) => (
                <span key={i} className="cert-tag"><Award size={14} />{c}</span>
              ))}
            </div>
          </section>
        )}
      ...
      <footer className="dp-footer">Built with <span className="grad" style={{ fontWeight: 700 }}>PortfolioAI</span> ✨</footer>
    </div>
  );
}
